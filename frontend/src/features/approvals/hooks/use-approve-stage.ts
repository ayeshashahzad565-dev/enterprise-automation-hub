...
/** Optimistically removes the decided stage from every cached inbox list immediately. */
export function useApproveStage() {
  const queryClient = useQueryClient();

  return useMutation({
    mutationFn: ({ stageId, input }: ApproveInput) => approvalService.approve(stageId, input),
