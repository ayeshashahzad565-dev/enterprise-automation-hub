...
def by_request_type(requests: Sequence[object]) -> dict[str, int]:
    ...
    """
    return CountAggregator(key_fn=lambda r: getattr(r, "request_type")).aggregate(requests)
...
def by_requester(requests: Sequence[object]) -> dict[UUID, int]:
    ...
    """
    return CountAggregator(key_fn=lambda r: getattr(r, "requester_id")).aggregate(requests)
...
def by_decider(stages: Sequence[object]) -> dict[UUID, int]:
    ...
    return CountAggregator(key_fn=lambda s: getattr(s, "decided_by")).aggregate(decided_only)
...
def by_actor(entries: Sequence[object]) -> dict[UUID, int]:
    ...
    return CountAggregator(key_fn=lambda e: getattr(e, "actor_id")).aggregate(with_actor)
...
def by_assignee(stages: Sequence[object]) -> dict[UUID, int]:
    ...
    return CountAggregator(key_fn=lambda s: getattr(s, "assigned_to")).aggregate(assigned_only)
